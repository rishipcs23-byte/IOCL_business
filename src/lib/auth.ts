...
export async function loginUser(username: string, passwordHash: string): Promise<SessionUser | null> {
  ...
  const cookieStore = await cookies();
  cookieStore.set('auth_user_id', sessionUser.id, { httpOnly: true, secure: isSecure, sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 });
  cookieStore.set('auth_role', sessionUser.role, { httpOnly: true, secure: isSecure, sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 });
  cookieStore.set('auth_username', sessionUser.username, { httpOnly: true, secure: isSecure, sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 });
